 ...
 * nov 13, LEC 16 (ab): Virtual memory for applications (baker example) 
 * 
 * This file is copied from:
 * https://pdos.csail.mit.edu/6.1810/2023/lec/baker.c
 * 
 * $ gcc baker.c -o baker
 * 
 * $ ./baker | less
...
#define TOFROM (2*SPACESZ)    // size of the to-space and the from-space
#define NROOT 2               // number of roots

// the space GC allocates in and copies to
static void *to; 
static void *to_free_start;  // the start address of the remaining free space in the to-space
static void *scanned;
...
flip() {
  ...
  if (mprotect(to, SPACESZ, PROT_NONE) < 0) {
    fprintf(stderr, "Couldn't unmap the to-space; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
#endif
...
// With VM: check if pointer should be forwarded.  Without VM, do
...
new(void) {
  ...
  if (collecting && scanned < to_free_start) {
    scan(scanned);
    /*
    Question: How "scanned" is updated? 
    Answer: there is a line of code : scanned = appaddr(next);  in scan()
    */
...
collector_thread(void *x) {
  ...
  while (!done) {
    ...
    if (collecting && scanned < to_free_start) {
      ...
      /*
      Question: How "scanned" is updated? 
      Ansewer: there is a line of code : scanned = appaddr(next);  in scan()
      */
      if (scanned >= to_free_start) {
        end_collecting();
      }
...
setup_spaces(void)
{
  ...
  if (mutator == MAP_FAILED) {
    fprintf(stderr, "Couldn't mmap() from+to space; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

  printf("Address where from+to space is mapped for mutator: %p\n", mutator);
  ...
  /*
  Caution: mutator and collector are 2 different addresses

  Address where from+to space is mapped for mutator:   0x70fb8edde000
  Address where from+to space is mapped for collector: 0x70fb8edd8000
  */
  printf("Address where from+to space is mapped for collector: %p\n", collector);
...
main(int argc, char *argv[])
{
  ...
  if (pthread_create(&the_app, NULL, app_thread,  NULL) != 0) {
    fprintf(stderr, "Couldn't start the app thread: %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

#ifdef VM
  if (pthread_create(&the_collector, NULL, collector_thread,  NULL) != 0) {
    fprintf(stderr, "Couldn't start the collector thread: %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
#endif
